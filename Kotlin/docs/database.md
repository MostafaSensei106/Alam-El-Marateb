...
## 14. جداول التسعير المتغيرة (V26)

- `meter_prices (product, shape, price>0, UQ)` — سعر المتر لكل موديل × شكل (مستطيل/بيضاوي/دائري)،
  ويسقط للـ `products.price_per_meter` العام عند غياب الصف.
- `operating_brackets (product nullable, width_from<=width_to, pct 0..100)` — صفوف product_id NULL
  هي الافتراضي العام؛ صفوف الموديل تُركّب فوقها (نطاق الموديل يكسب في مداه فقط).
- بلا حدود على المقاسات: خارج كل الشرائح = 0% (لا رفض أبداً).
...
V26 جداول التسعير المتغيرة (مُنفذ).
